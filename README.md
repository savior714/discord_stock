...
- **대규모 티커 감시 시**: 500개 종목 감시 시 10분마다 모든 종목을 체크하므로 네트워크 트래픽이 발생할 수 있습니다
- **Discord 메시지 제한**: 알림이 많을 경우 10초 간격으로 전송되므로 모든 알림이 전송되기까지 시간이 소요될 수 있습니다

## 알려진 문제 및 해결 방법

### 일부 티커 다운로드 실패 또는 MFI 계산 오류

**증상**: 
- 일부 티커에서 간헐적으로 실패 또는 MFI가 NaN으로 표시
- 예: COF 티커에서 MFI 계산 실패
...
5. **MFI 계산 오류**: 극단적인 상승세로 negative money flow가 0일 때 0으로 나누기 발생

**해결 방법**:
- ✅ **자동 재시도**: 최대 2회 자동 재시도 (이미 구현됨)
- ✅ **상세 로깅**: 실패한 티커와 원인을 명확하게 표시
- ✅ **MFI 0으로 나누기 방지**: negative flow가 0일 때 매우 작은 값으로 대체하여 MFI=100으로 계산
- ✅ **특수 문자 자동 처리**: `BRK.B` 실패 시 자동으로 `BRK-B` 시도
- 📝 **수동 확인**: 로그에서 실패한 티커를 확인하고 필요시 목록에서 제거

**참고**: 99%+ 성공률은 정상적인 범위이며, 실제 운영 환경에서 매우 우수한 수준입니다.
